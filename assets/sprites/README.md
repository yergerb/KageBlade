...
The original sheet is arranged as a rough 4 by 3 grid. The idle wind atlas is generated from the cleaned shoulder-sword pose so Ren stays planted while the clothing and sword move. The duck atlas is generated from the cleaned crouch guard pose so Down has a readable low profile. Combat sheet 02 has a normalized atlas and manifest, and the fighter controller uses it for walk forward, walk backward, dash forward, back evade, kick, and forward thrust.
